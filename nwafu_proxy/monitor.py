...
def _extract_capabilities(item: dict) -> dict:
    ...
    return {
        k: caps.get(k, False)
        for k in ("vision", "file_upload", "web_search", "image_generation", "code_interpreter")
    }
...
def _parse_models(data: list[dict]) -> dict[str, ModelInfo]:
    ...
    for item in data:
        ...
        result[mid] = ModelInfo(
            ...
            model_type=_detect_model_type(item),
...
class ModelMonitor:
    ...
    def __init__(
        ...
        *,
        ...
        fetch_models: Callable[[], Awaitable[dict]] | None = None,
    ):
    ...
    async def fetch_models(self) -> dict[str, ModelInfo]:
        if self._fetch_models is not None:
            ...
            return _parse_models(data.get("data", []))
    ...
    async def poll_once(self) -> Optional[ModelDiff]:
        ...
        if not self._initialized:
            ...
            logger.info(
                "模型基线建立：%d 个模型 [%s]", len(new_models), ", ".join(sorted(new_models))
            )
